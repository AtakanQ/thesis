function [laneBorders,laneCenters] = retrieveHERE_v3(folderName,HEREname,refLat,refLon,lats,lons)
...
for i = 1:size(laneCenterData,2) % for each lane
    if( ~isempty(laneCenterData{1,i}))
        laneCenter.lats = [];
        laneCenter.lons = [];
        start_lat = laneCenterData{1,i}(1,1); % might have to look at other end as well
        start_lon = laneCenterData{1,i}(1,2);
    
        laneCenter.lats = flip(laneCenterData{1,i}(:,1));
        laneCenter.lons = flip(laneCenterData{1,i}(:,2));
        ...
        for j = 2:size(laneCenterData,1)
            for k = 1:size(laneCenterData,2)
                if( ~isempty(laneCenterData{j,k}))
                    target_lat = laneCenterData{j,k}(1,1);
                    target_lon = laneCenterData{j,k}(1,2);
                    tempDist = distance(start_lat,start_lon,target_lat,target_lon,wgs84Ellipsoid);
                    if(tempDist < distanceThreshold)
                        foundFirstTime = true;
                        laneCenter.lats = [laneCenter.lats; laneCenterData{j,k}(:,1)];
                        laneCenter.lons = [laneCenter.lons; laneCenterData{j,k}(:,2)];
                        start_lat = laneCenterData{j,k}(end,1);
                        start_lon = laneCenterData{j,k}(end,2);
                        break
                    end
        
                    target_lat = laneCenterData{j,k}(end,1);
                    target_lon = laneCenterData{j,k}(end,2);
                    tempDist = distance(start_lat,start_lon,target_lat,target_lon,wgs84Ellipsoid);
                    if(tempDist < distanceThreshold)
                        foundFirstTime = true;
                        laneCenter.lats = [laneCenter.lats; flip(laneCenterData{j,k}(:,1))];
                        laneCenter.lons = [laneCenter.lons; flip(laneCenterData{j,k}(:,2))];
                        start_lat = laneCenterData{j,k}(1,1);
                        start_lon = laneCenterData{j,k}(1,2);
                        break
                    end
                end
            end
       ...
       if(~foundFirstTime && (j==2) )% failed to concatenate the first piece
            start_lat = laneCenterData{1,i}(end,1); % might have to look at other end as well
            start_lon = laneCenterData{1,i}(end,2);
        
            laneCenter.lats = laneCenterData{1,i}(:,1);
            laneCenter.lons = laneCenterData{1,i}(:,2);
            for j = 2:size(laneCenterData,1)
                for k = 1:size(laneCenterData,2)
                    if( ~isempty(laneCenterData{j,k}))
                        target_lat = laneCenterData{j,k}(1,1);
