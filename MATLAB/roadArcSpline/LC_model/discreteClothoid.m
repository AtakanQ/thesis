function [CircleData,LineData] = discreteClothoid(S,n,kA,kB,PS,thetaS,plotOn)
lineColor = 'k';
k = kB-kA;
h = (kB-kA)/n;
lane = 4;
...
%     plot([0 X1],[Y1+lane Y1+lane])
    ...
    if kA > 0
        ...
        phi = unique([w0+thetaS:.0001:thetaS thetaS]);
    end
    AX = CX+R0*sin(phi);
    AY = CY-R0*cos(phi);
...
for j = 2:n
    ...
    if wVec(j) == 0
        if j < n
            AX = CX+[0:.001:S/n]*cos(thetaS+w0+sum(wVec(1:j-1)));
            AY = CY+[0:.001:S/n]*sin(thetaS+w0+sum(wVec(1:j-1)));
        else
            AX = CX+[0:.001:S/n/2]*cos(thetaS+w0+sum(wVec(1:j-1)));
            AY = CY+[0:.001:S/n/2]*sin(thetaS+w0+sum(wVec(1:j-1)));
        end
...
% theta_end = phi(end);
% SP = [AX(1) AY(1)];
% EP = [SP(1)+500*cos(theta_end) SP(2)+500*sin(theta_end)];
% 
