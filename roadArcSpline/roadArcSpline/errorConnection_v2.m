...
% Position error from heading/curvature correction + initial distance error
deltaY = clo.allY(end) + y0; 
lambda = 0.5; 
gamma = 0.5; 
...
% Plot the curvature of the bi-elementary path
k2 = -lambda/(1-lambda)*k1; 
SE1 = lambda*gamma*l; 
SE2 = (1-lambda)*gamma*l; 
SL = (1-gamma)*l; 
...
subplot(3,1,3)
plot(clo.allX,clo.allY+y0,'LineWidth',2)
hold all; 
plot(xVec,yVec,'LineWidth',2)
plot(all_x,clo_y+bi_y+y0,'LineWidth',2)
grid; 
xlabel('x-position [m]')
ylabel('y-position [m]')
